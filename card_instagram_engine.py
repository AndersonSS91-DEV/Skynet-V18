...
#
# IMPORTANTE (deploy no Streamlit Cloud / qualquer servidor):
# o servidor não tem as fontes do sistema que existem no seu PC.
# Por isso este arquivo carrega as fontes de uma pasta "fonts/"
# que deve ficar AO LADO deste .py (mesmo diretório). Baixe o
# fonts.zip que te mandei junto, extraia, e garanta que a
# estrutura fique assim no seu repositório:
#
#   seu_projeto/
#     app.py
#     card_instagram_engine.py
...
#       NotoColorEmoji.ttf
#
# Se a pasta fonts/ não existir, o motor tenta caminhos comuns do
# sistema e, por fim, a fonte padrão do Pillow — os emojis coloridos
# simplesmente não aparecem se o NotoColorEmoji não for encontrado
# em nenhum lugar (o card continua funcionando, só sem os ícones).
# ============================================================
import io
import os
import re
...
_EMOJI_CANDIDATES = [
    os.path.join(_BASE_DIR, "fonts", "NotoColorEmoji.ttf"),
    "/usr/share/fonts/truetype/noto/NotoColorEmoji.ttf",
]

# fallback extra: fonte DejaVu que já vem embutida no matplotlib
# (quase sempre presente em apps de dados) — evita depender só da
# pasta fonts/ se por algum motivo ela não foi incluída no deploy.
try:
    import matplotlib
    ...
    for path in candidates:
        if os.path.exists(path):
            try:
                ...
                continue
    # último recurso: fonte padrão do Pillow (bitmap, tamanho fixo em
    # versões antigas; em Pillow >=10.1 aceita size=)
    try:
...
def _emoji_font():
    for path in _EMOJI_CANDIDATES:
        if os.path.exists(path):
            try:
                return ImageFont.truetype(path, EMOJI_NATIVE_SIZE)
            except Exception:
                continue
    return None
...
def _render_emoji_glyph(ch, size):
    ...
    Retorna None se não houver fonte de emoji disponível."""
    ef = _emoji_font()
    if ef is None:
        return None
    ...
    try:
        td.text((15, 8), ch, font=ef, embedded_color=True)
    except Exception:
        return None
    bbox = tmp.getbbox()
    if not bbox:
...
def draw_emoji_str(base_img, s, xy, size, anchor="mm", gap=2):
    ...
    Se a fonte de emoji não estiver disponível, não desenha nada
    (retorna 0) em vez de quebrar o card."""
